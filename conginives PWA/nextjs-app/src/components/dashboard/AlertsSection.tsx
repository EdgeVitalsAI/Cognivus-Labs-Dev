                ...
                {alerts.length === 0 ? (
                    ...
                    <div className="space-y-2.5">
                        {alerts.map((alert, index) => (
                            <div
                                key={index}
                                className={`flex items-start p-3 rounded-lg border-l-[3px] transition-all duration-300 animate-slide-in-right ${alert.type === 'critical'
                                        ? 'bg-accent-red/[0.06] border-accent-red'
                                        : 'bg-accent-amber/[0.06] border-accent-amber'
                                    }`}
                                style={{ animationDelay: `${index * 0.1}s` }}
                            >
                                <div className={`w-8 h-8 rounded-lg flex items-center justify-center mr-2.5 flex-shrink-0 ${alert.type === 'critical'
                                        ? 'bg-accent-red/15'
                                        : 'bg-accent-amber/15'
                                    }`}>
                                    <i
                                        className={`fas ${alert.type === 'critical' ? 'fa-exclamation-triangle' : 'fa-exclamation-circle'
                                            } text-sm ${alert.type === 'critical' ? 'text-accent-red' : 'text-accent-amber'
                                            }`}
                                    ></i>
                                </div>
