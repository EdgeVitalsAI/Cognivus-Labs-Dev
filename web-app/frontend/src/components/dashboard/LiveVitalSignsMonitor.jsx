import { AlertCircle, Droplet, Heart, Maximize2, Pause, Thermometer, Wind } from 'lucide-react';
...
const LiveVitalSignsMonitor = () => {
        ...
        const interval = setInterval(() => {
            setVitals((prev) => ({
                ...
                temperature: Math.max(
                    97,
                    Math.min(101, prev.temperature + (Math.random() - 0.5) * 0.5)
                ),
                bloodPressure: {
                    systolic: Math.max(
                        100,
                        Math.min(180, prev.bloodPressure.systolic + (Math.random() - 0.5) * 8)
                    ),
                    diastolic: Math.max(
                        60,
                        Math.min(120, prev.bloodPressure.diastolic + (Math.random() - 0.5) * 8)
                    ),
                },
    ...
    const getVitalStatus = (type, value) => {
        switch (type) {
            case 'heartRate':
                if (value < 60)
                    return { label: 'LOW', color: 'text-blue-400', bg: 'bg-blue-900/30' };
                if (value < 100)
                    return { label: 'NORMAL', color: 'text-green-400', bg: 'bg-green-900/30' };
                if (value < 120)
                    return { label: 'WARNING', color: 'text-yellow-400', bg: 'bg-yellow-900/30' };
            ...
            case 'bloodPressure':
                ...
                if (value.systolic < 140 && value.diastolic < 90)
                    return { label: 'ELEVATED', color: 'text-yellow-400', bg: 'bg-yellow-900/30' };
                return { label: 'CRITICAL', color: 'text-red-400', bg: 'bg-red-900/30' };
            case 'spO2':
                if (value >= 95)
                    return { label: 'NORMAL', color: 'text-green-400', bg: 'bg-green-900/30' };
    ...
    const points = heartRateHistory
        .map(
            (rate, i) =>
                `${(i / (heartRateHistory.length - 1)) * 100},${
                    100 - ((rate - minHeartRate) / scale) * 80
                }`
        )
        .join(' ');
            ...
            <div className="grid grid-cols-4 gap-4 mb-6">
                ...
                <div className={`${bpStatus.bg} border border-slate-700 rounded-lg p-4`}>
                    <div className="flex items-center justify-between mb-3">
                        ...
                        <span className={`text-sm font-semibold ${heartRateStatus.color}`}>
                ...
                <div className={`${bpStatus.bg} border border-slate-700 rounded-lg p-4`}>
                    <div className="flex items-center justify-between mb-3">
                        ...
                        <span className={`text-sm font-semibold ${bpStatus.color}`}>
                            {bpStatus.label}
                        </span>
                    </div>
                    <div className={`text-3xl font-bold ${bpStatus.color}`}>
                        {Math.round(vitals.bloodPressure.systolic)}/
                ...
                <div className={`${spO2Status.bg} border border-slate-700 rounded-lg p-4`}>
                    <div className="flex items-center justify-between mb-3">
                        ...
                        <span className={`text-sm font-semibold ${spO2Status.color}`}>
                            {spO2Status.label}
                        </span>
                    </div>
                    <div className={`text-3xl font-bold ${spO2Status.color}`}>
