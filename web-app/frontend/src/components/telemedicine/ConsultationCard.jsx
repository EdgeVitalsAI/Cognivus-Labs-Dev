...
const ConsultationCard = ({ consultation, onJoin, onReschedule, onCancel }) => {
    ...
    return (
        <div
            className={`${status.bg} border-l-4 ${status.border} bg-gradient-to-r from-slate-800 to-slate-900 rounded-lg border border-slate-700 p-4 hover:shadow-lg hover:shadow-slate-700/50 transition-all`}
        >
                        ...
                        {consultation.symptoms.length > 0 && (
                            <div className="flex flex-wrap gap-2">
                                {consultation.symptoms.map((symptom, idx) => (
                                    <span
                                        key={idx}
                                        className="text-xs bg-slate-600 text-slate-200 px-2.5 py-1 rounded-full"
                                    >
                                        {symptom}
                                    </span>
                                ))}
