import { Wifi, Battery, Signal, AlertTriangle, RefreshCw, Trash2, CheckCircle } from 'lucide-react'

const DeviceCard = ({ device, onResync, onUnpair }) => {
  const getStatusColor = (status) => {
    switch (status) {
      case 'ACTIVE':
        return { bg: 'bg-emerald-500/10', border: 'border-emerald-500', text: 'text-emerald-400', icon: CheckCircle }
      case 'LOW_BATTERY':
        return { bg: 'bg-amber-500/10', border: 'border-amber-500', text: 'text-amber-400', icon: AlertTriangle }
      case 'INACTIVE':
        return { bg: 'bg-red-500/10', border: 'border-red-500', text: 'text-red-400', icon: AlertTriangle }
      default:
        return { bg: 'bg-slate-500/10', border: 'border-slate-500', text: 'text-slate-400', icon: AlertTriangle }
    }
  }
  ...
  return (
    <div className={`${status.bg} border-l-4 ${status.border} bg-gradient-to-br from-slate-800 to-slate-900 rounded-lg border border-slate-700 p-4 hover:shadow-lg hover:shadow-slate-700/50 transition-all`}>
      {/* Header */}
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        ...
        <div className="bg-slate-700/30 rounded-lg p-3 border border-slate-700">
          ...
          <div className="flex items-center gap-1 mb-1">
            {[...Array(4)].map((_, i) => (
              <div
                key={i}
                className={`flex-1 h-2 rounded-full ${i < signalBars ? 'bg-sky-400' : 'bg-slate-600'}`}
              ></div>
            ))}
          </div>
        ...
        <div className="bg-slate-700/30 rounded-lg p-3 border border-slate-700">
          ...
          <p className="text-sm font-semibold text-white">{formatLastSync(device.lastSync)}</p>
        </div>
      </div>

      {/* Sensors Grid */}
      <div className="mb-4">
        <p className="text-xs text-slate-400 uppercase tracking-wider mb-2">Sensor Status</p>
        <div className="space-y-2">
          {device.sensors.map((sensor, idx) => (
            <div key={idx} className="flex items-center justify-between p-2 bg-slate-700/20 rounded-lg border border-slate-700">
              <div className="flex items-center gap-2">
                <div className={`w-2 h-2 rounded-full ${
                  sensor.status === 'healthy' ? 'bg-emerald-500' :
                  sensor.status === 'warning' ? 'bg-amber-500' :
                  'bg-red-500'
                }`}></div>
                <span className="text-sm text-slate-300">{sensor.name}</span>
              </div>
              <span className="text-sm font-semibold text-slate-200">{sensor.value}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="mb-4 p-3 bg-slate-700/20 rounded-lg border border-slate-700">
        ...
        <div className="w-full h-2 bg-slate-700 rounded-full overflow-hidden">
          <div
            className={`h-full rounded-full ${device.storageUsed > 80 ? 'bg-red-500' : device.storageUsed > 50 ? 'bg-amber-500' : 'bg-sky-500'}`}
      ...
      <div className="text-xs text-slate-400 mb-4 space-y-1">
        ...
        {device.alerts > 0 && <p className="text-amber-400">Alert: {device.alerts} alert{device.alerts > 1 ? 's' : ''}</p>}
      </div>
