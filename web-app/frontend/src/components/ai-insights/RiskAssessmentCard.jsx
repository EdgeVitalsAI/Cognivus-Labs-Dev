...
const RiskAssessmentCard = ({ assessment, onViewPatient }) => {
    ...
    return (
        <div
            className={`${overallRisk.bg} border ${overallRisk.border} rounded-xl p-6 bg-gradient-to-br from-slate-800 to-slate-900`}
        >
            {/* Header */}
            <div className="flex items-center justify-between mb-6">
                ...
                <div
                    className={`flex flex-col items-center justify-center w-20 h-20 rounded-full border-4 ${overallRisk.border} ${overallRisk.bg}`}
                >
                    <p className={`text-2xl font-bold ${overallRisk.text}`}>
                    ...
                    {assessment.riskFactors.map((factor, idx) => (
                        ...
                        >
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <span className="text-sm font-semibold text-slate-200 w-8">
                                    {factor.score}
                                </span>
            ...
            <div className="border-t border-slate-700 pt-6">
                ...
                <div className="space-y-2">
                    {assessment.predictedOutcomes.map((outcome, idx) => (
                        <div
                            key={idx}
                            className="flex items-center justify-between p-3 bg-slate-700/20 rounded-lg border border-slate-700"
                        >
