import { AlertCircle, CheckCircle, TrendingUp, ChevronRight } from 'lucide-react'

const AIInsightCard = ({ insight, onMarkAddressed, onViewPatient }) => {
  const getSeverityColor = (severity) => {
    switch (severity) {
      case 'HIGH':
        return { bg: 'bg-red-500/10', border: 'border-red-500', text: 'text-red-400', icon: AlertCircle }
      case 'MEDIUM':
        return { bg: 'bg-amber-500/10', border: 'border-amber-500', text: 'text-amber-400', icon: TrendingUp }
      case 'LOW':
  ...
  return (
    <div className={`${severity.bg} border-l-4 ${severity.border} bg-gradient-to-r from-slate-800 to-slate-900 rounded-lg border border-slate-700 p-4 hover:shadow-lg hover:shadow-slate-700/50 transition-all`}>
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1">
          {/* Header with Type and Severity */}
          <div className="flex items-center gap-3 mb-3">
            <SeverityIcon className={`w-5 h-5 ${severity.text}`} />
            <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${getTypeColor(insight.type)} capitalize`}>
              {insight.type}
            </span>
            <span className={`text-xs font-bold px-2.5 py-1 rounded-full border ${severity.border} ${severity.text} ${severity.bg}`}>
