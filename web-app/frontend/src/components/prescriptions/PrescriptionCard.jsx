import { CheckCircle, Clock, AlertCircle, MoreVertical, ChevronRight } from 'lucide-react'
...
const PrescriptionCard = ({ prescription, onViewPatient, onEdit, onRefill }) => {
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'SCHEDULED':
        return { bg: 'bg-amber-500/10', border: 'border-amber-500', text: 'text-amber-400', icon: Clock }
      case 'DISCONTINUED':
        return { bg: 'bg-slate-500/10', border: 'border-slate-500', text: 'text-slate-400', icon: AlertCircle }
      default:
        return { bg: 'bg-slate-500/10', border: 'border-slate-500', text: 'text-slate-400', icon: AlertCircle }
    }
  }
  ...
  return (
    <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-xl border border-slate-700 overflow-hidden hover:border-sky-500 transition-all hover:shadow-lg hover:shadow-sky-500/20">
      {/* Header */}
      <div className={`${status.bg} border-b ${status.border} p-4 flex items-start justify-between`}>
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            <h3 className="text-lg font-bold text-white">{prescription.medicationName}</h3>
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          {/* Prescribed By */}
          <div className="bg-slate-700/20 rounded-lg p-3 border border-slate-700">
            <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">Prescribed By</p>
            <p className="text-white font-semibold text-sm">{prescription.prescribedBy}</p>
          </div>

          {/* Adherence */}
          <div className={`rounded-lg p-3 border ${prescription.adherence !== 'N/A' ? 'bg-emerald-500/10 border-emerald-500' : 'bg-slate-700/20 border-slate-700'}`}>
            <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">Adherence</p>
            <p className={`font-semibold text-sm ${prescription.adherence !== 'N/A' ? 'text-emerald-400' : 'text-white'}`}>
        ...
        <div className="bg-slate-700/20 rounded-lg p-3 border border-slate-700 space-y-2">
          <div className="flex justify-between text-sm">
            ...
            <span className="text-white font-semibold">{prescription.lastDispensed}</span>
          </div>
          <div className="flex justify-between text-sm">
          ...
          <div className="flex justify-between text-sm border-t border-slate-600 pt-2">
            ...
            <span className={`font-semibold ${prescription.refillsRemaining > 0 ? 'text-emerald-400' : 'text-red-400'}`}>
